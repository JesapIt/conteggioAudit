...
options = ['Call d\'area', 'Assemblea mensile', 'Call con HR buddy', 'Board Resp/Resp Vice', 'Evento', 'Progetto Interno', 'Task Interno', 'Team Google Sheets', 'Team PowerBI', 'Team Make', 'Team BPMN', 
	   'Auditor', 'Report d\'area', 'Report delega', 'Formazione', 'Mentoring', 'Recruiting', 'Progetto esterno', 'Auditing progetto', 'Organizzazione area', 'Revisione task', 'Delega', 'Altro', 'Business Game']
...
for a in att:
	...
	if a == "Progetto esterno":
		...
		for name, value, state in zip(column_b, column_d, column_e):
			if (value.lower() == 'in corso' and state.lower() == 'progetto esterno') or (value.lower() == 'in avvio' and state.lower() == 'progetto esterno'):
